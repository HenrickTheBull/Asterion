...
class AsterConfig(Config):

    NAME = "aster"
    PATH = "cfg/core/aster.yaml"

    def __init__(self):
        super().__init__()
        self.prefixes = [os.getenv("ASTER_PREFIX", *self.prefixes)]
...
class AsterEmotesConfig(Config):

    NAME = "emotes"
    PATH = "cfg/core/emotes.yaml"


class AsterColorScheme(Config):
...
class AsterImagesConfig(Config):
...
class AsterMetaInformation(Config):
